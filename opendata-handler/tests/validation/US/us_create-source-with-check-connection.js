...
async function main() {
    ...
    for (let p = 0; p < rp_len; p++) {
        ...
        while (i < count) {
            ...
            const sourceId = await validate.validateFinal(csvSourceInfo)
            if (sourceId == null) {
        ...
        console.log(`number of created sources in ${publisher} catalog ${pg}: ${cnt}`)
    ...
    console.log(`Number of created sources with connection check in US portal ${format} files: ${global_cnt}`)
